...
var validateGalleryAdmin = validator.New()

var customErrorMessagesGalleryAdmin = map[string]string{
	"Year.required":     "Year is required",
	"Year.min":          "Year must be at least 3 characters long",
	"Category.required": "Category is required",
}

type AdminGalleryControllerCP struct {
	GalleryService *services.GalleryService
}

// NewAdminGalleryControllerCP initializes the controller and handles any errors from NewGalleryService
func NewAdminGalleryControllerCP() *AdminGalleryControllerCP {
	...
	return &AdminGalleryControllerCP{GalleryService: galleryService}
}

// GetGallery handles fetching a gallery by slug
func (c *AdminGalleryControllerCP) GetGallerys(ctx *fiber.Ctx) error {
...
func (c *AdminGalleryControllerCP) GetGalleryByUUID(ctx *fiber.Ctx) error {
	...
	hasAccess, err := enforcer.Enforce(username, "gallery", "read", "none", "none", "none")
	if err != nil {
...
func (c *AdminGalleryControllerCP) GetGallerysPaginated(ctx *fiber.Ctx) error {
	// Get the username from the context (set by the JWT middleware)
	username := ctx.Locals("username").(string) // Use ctx.Locals instead of c.Locals
...
func (c *AdminGalleryControllerCP) CreateGallery(ctx *fiber.Ctx) error {
	...
	if err := validateGalleryAdmin.Struct(gallery); err != nil {
		...
		for _, err := range validationErrors {
			...
			// Get custom error message from customErrorMessagesGalleryAdmin or use a default message
			if customMsg, exists := customErrorMessagesGalleryAdmin[errorKey]; exists {
...
func (c *AdminGalleryControllerCP) UpdateGallery(ctx *fiber.Ctx) error {
	// Get the username from the context (set by the JWT middleware)
	...
	hasAccess, err := enforcer.Enforce(username, "gallery", "update", "none", "none", "none")
	if err != nil {
		return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"statusCode": fiber.StatusInternalServerError,
			"message":    "Failed to check access.",
	...
	// Validate the input data
	if err := validateGalleryAdmin.Struct(updatedAdminGallery); err != nil {
		// If validation fails, return the validation error messages
		validationErrors := err.(validator.ValidationErrors)
		errors := make(map[string]string)
...
func (c *AdminGalleryControllerCP) DeleteGallery(ctx *fiber.Ctx) error {
	// Get the username from the context (set by the JWT middleware)
	username := ctx.Locals("username").(string) // Use ctx.Locals instead of c.Locals
	...
	// Check if the user has access to the "/admin" resource using the "GET" action
	hasAccess, err := enforcer.Enforce(username, "gallery", "delete", "none", "none", "none")
	if err != nil {
		return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{ // Use ctx.Status
			"statusCode": fiber.StatusInternalServerError,
			"message":    "Failed to check access.",
