...
export async function GET(request: Request) {
  try {
    ...
    const api = `${process.env.NEXT_PUBLIC_API_URL}user/document-revision/published?perPage=10&page=1&sortBy=id&sortDesc=true&showAll=true&document_control_uuid=${uuid}`;
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Successfully get document history",
        data: response.data
      },
      {
