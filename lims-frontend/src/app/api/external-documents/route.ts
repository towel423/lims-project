...
export async function GET(request: Request) {
  try {
    ...
    const api = `${process.env.NEXT_PUBLIC_API_URL}user/document-control/list/external?perPage=${per_page}&page=${page}&sortBy=id&sortDesc=true&document_type_id=${document_type}&status_document_id=${document_status}`;

    // Make the API request
    const response = await axios.get<ExternalDocumentsType[]>(api, {
      headers: {
